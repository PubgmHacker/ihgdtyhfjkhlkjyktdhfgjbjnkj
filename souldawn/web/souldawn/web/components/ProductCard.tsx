...
export default function ProductCard({ product, layout = "grid", onProductClick }: ProductCardProps) {
  ...
  // Остаток: stock=0 — нет в наличии; 1..5 — показываем «Осталось N» с анимацией.
  const hasStockField = typeof product.stock === "number";
  const isSoldOut = product.tag === "Нет в наличии" || (hasStockField && (product.stock as number) <= 0);
  const lowStock = hasStockField && (product.stock as number) > 0 && (product.stock as number) <= 5;
  // Угловой круглый бейдж: приоритет у product.badge, иначе выводим из tag.
  const badge: "NEW" | "HIT" | "SALE" | null =
    product.badge ||
    (product.tag === "Новинка" ? "NEW" : product.tag === "Хит" ? "HIT" : product.tag === "Скидка" ? "SALE" : null);
  const badgeLabel = badge === "NEW" ? "NEW" : badge === "HIT" ? "ХИТ" : badge === "SALE" ? "SALE" : "";
  const badgeClass =
    badge === "SALE"
      ...
      ? "bg-[#E0B341] text-bg"
      : "bg-accent text-bg";
  const photos = product.images && product.images.length ? product.images : product.image ? [product.image] : [];
  ...
  return (
    ...
    >
      {/* Image area */}
      <div className={`aspect-[3/4] bg-gradient-to-br ${product.gradient} relative overflow-hidden ${isSoldOut ? "opacity-90" : ""}`}>
        {/* Реальные фото (перёд/зад) или fallback-иконка */}
        {photos.length ? (
          <>
            <img
              src={photos[0]}
              ...
              className={`absolute inset-0 w-full h-full object-cover transition-all duration-700 group-hover:scale-105 ${
                photos[1] && hovered ? "opacity-0" : "opacity-100"
              }`}
            />
            {photos[1] && (
              <img
        ...
        {/* Угловой круглый бейдж (правый верхний угол) */}
        {badge && (
          <span
            className={`absolute top-3 right-3 z-20 w-11 h-11 rounded-full flex items-center justify-center text-[9px] font-black tracking-wider shadow-lg ${badgeClass}`}
          >
            {badgeLabel}
          </span>
        )}
        ...
        {!isSoldOut && lowStock && (
          <motion.span
            className="absolute bottom-3 left-3 z-20 text-[9px] font-bold tracking-wider uppercase px-2.5 py-1 bg-bg/70 backdrop-blur-sm text-accent border border-accent/30"
        ...
        {isSoldOut && (
          <span className="absolute bottom-3 left-3 z-20 text-[9px] font-bold tracking-wider uppercase px-2.5 py-1 bg-bg/70 backdrop-blur-sm text-white/50 border border-white/15">
            Нет в наличии
          </span>
        )}

...
from database import get_or_create_user, create_ticket, update_ticket_admin_messages
...
from keyboards import main_kb, back_kb, operator_confirm_kb, ai_helpful_kb
...
async def handle_support_msg(message: Message, state: FSMContext, bot: Bot):
    ...
    if ADMIN_IDS:
        try:
            ...
            # Step 2: Send messages to admins with real ticket_id in callback_data
            # Use bot.send_message (NOT message.copy_to — caption is ignored for text messages)
            admin_messages = []
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📥 Взять в работу", callback_data=f"take_ticket:{ticket_id}")]
            ])

  ...
  useEffect(() => {
    if (typeof window !== "undefined" && (window as any).Telegram?.WebApp) {
      ...
      if (user?.id) {
        ...
        setUserProfile({ id: user.id.toString(), username: user.username, first_name: user.first_name, photo_url: user.photo_url });
      } else {
        setTgId("8340654471");
        setUserProfile({ id: "8340654471", username: "shoppingpharmaa", first_name: "Soul", photo_url: "" });
      }
    }
  }, []);
      ...
      {selectedTicket ? (
        <div className="flex flex-col h-[calc(100vh-140px)] border border-zinc-800 bg-zinc-950/60 p-2 relative rounded-sm">
          <div className="flex justify-between items-center border-b border-zinc-800 pb-2 mb-2">
            <button onClick={() => setSelectedTicket(null)} className="text-amber-500 text-xs font-bold bg-transparent border-none cursor-pointer">[ ← НАЗАД ]</button>
            <span className="text-[9px] text-zinc-500 uppercase">ТИКЕТ #{selectedTicket.id.slice(-6)}</span>
          </div>
          <div className="flex-1 overflow-y-auto space-y-2 p-2 text-xs">
            <div className="bg-zinc-900/40 p-2 border border-zinc-800 text-zinc-400 border-l-2 border-l-amber-500 mb-3">{selectedTicket.message}</div>
          ...
          <form onSubmit={sendChatMessage} className="flex gap-2 border-t border-zinc-800 pt-2 mt-2">
            <input type="text" value={chatText} onChange={(e) => setChatText(e.target.value)} placeholder="Сообщение..." className="flex-1 bg-zinc-900 border border-zinc-800 p-2 text-xs text-white focus:outline-none focus:border-amber-500 rounded-sm" />
            <button type="submit" className="bg-amber-500 text-black font-black text-xs px-4 border-none rounded-sm">ОТПРАВИТЬ</button>
          </form>
        </div>
      ) : (
        <>
          <div className="flex border border-zinc-800 bg-zinc-900/40 mb-4 p-1 rounded-sm">
            <button onClick={() => setActiveTab("create")} className={`flex-1 py-3 text-center text-xs uppercase font-black border-none cursor-pointer ${activeTab === "create" ? "bg-amber-500 text-black" : "text-zinc-500"}`}>[ СОЗДАТЬ ]</button>
            <button onClick={() => setActiveTab("history")} className={`flex-1 py-3 text-center text-xs uppercase font-black border-none cursor-pointer ${activeTab === "history" ? "bg-amber-500 text-black" : "text-zinc-500"}`}>[ ИСТОРИЯ ({history.length}) ]</button>
          </div>
          {activeTab === "create" ? (
            <form onSubmit={handleSubmit} className="space-y-4">
              <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 p-3 text-xs text-zinc-200 focus:outline-none focus:border-amber-500 rounded-sm"><option value="general">ОБЩИЕ ВОПРОСЫ</option><option value="order">ЗАКАЗЫ</option></select>
              <textarea required rows={5} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Описание проблемы..." className="w-full bg-zinc-900 border border-zinc-800 p-3 text-xs text-zinc-200 focus:outline-none focus:border-amber-500 resize-none rounded-sm" />
              <button type="submit" disabled={loading} className="w-full bg-amber-500 text-black font-black py-4 uppercase text-xs rounded-sm">{loading ? "⚡ СИНХРОНИЗАЦИЯ..." : "⚙️ ОТПРАВИТЬ ЗАПРОС"}</button>
            </form>
          ) : (
            <div className="space-y-2">
              {history.length === 0 ? ( <div className="text-center text-zinc-600 text-xs">// Нет сессий.</div> ) : (
                history.map((ticket: any) => (
                  <div key={ticket.id} onClick={() => setSelectedTicket(ticket)} className="bg-zinc-900/20 border border-zinc-800 p-3 rounded-sm border-l-2 border-l-amber-500 cursor-pointer">
                    <div className="flex justify-between items-center text-[9px]"><span className="text-zinc-400 font-bold">{ticket.category}</span><span className="text-amber-400">{ticket.status}</span></div>
                    <p className="text-xs text-zinc-300 truncate mt-1">{ticket.message}</p>
                  </div>
                ))
              )}
            </div>

...
/**
 * Social login buttons for Apple and VK.
 * OAuth not yet configured — shows a message when clicked.
 */
  ...
  return (
    <div className="w-full space-y-3">
      <button
        ...
        className="w-full flex items-center justify-center gap-2 border border-[rgba(200,200,210,0.14)] px-4 py-3 text-sm font-bold text-[#E8E8F0] hover:border-[rgba(200,200,210,0.3)] transition-colors"
      >
        <svg
          width="16"
          height="16"
      ...
      {msg && (
        <p className="text-xs text-[#6B6B78] text-center">{msg}</p>
      )}
    </div>
  );
}
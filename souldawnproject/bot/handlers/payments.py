...
async def poll_paid_orders(bot: Bot) -> None:
    ...
    while True:
        ...
        for order in orders:
            ...
            # Уведомить админов в main-боте (только транзакции)
            try:
                from bot_logger import log_payment_confirmed
                username = contact.get("name") or contact.get("phone") or "—"
                await log_payment_confirmed(
                    order_id=order_id,
                    total_kopecks=total,
                    username=username,
                    telegram_id=int(buyer_tg) if buyer_tg else None,
                    bot=bot,
                )
            except Exception as e:
                logger.warning(f"poll_paid_orders: log_payment_confirmed failed: {e}")

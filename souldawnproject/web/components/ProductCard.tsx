...
export default function ProductCard({ product, layout = "grid", onProductClick }: ProductCardProps) {
  ...
  return (
    ...
    >
      {/* Image area — клик по фото открывает модал */}
      <div
        className={`aspect-[3/4] bg-gradient-to-br ${product.gradient} relative overflow-hidden rounded-xl transition-shadow duration-500 group-hover:shadow-glow cursor-pointer`}
        onClick={handleCardClick}
      >
        {/* Фото: cross-fade спереди/сзади по hover */}
        ...
        {/* Индикатор двух фото (спереди/сзади) */}
        {product.images?.[1] && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
            <span className={`block w-4 h-[2px] transition-all duration-300 ${
              !hovered ? "bg-white/80" : "bg-white/30"
            }`} />
            <span className={`block w-4 h-[2px] transition-all duration-300 ${
              hovered ? "bg-accent" : "bg-white/30"
            }`} />
          </div>
        )}
        ...
        {/* Hover overlay — градиент снизу (не перекрывает клик по фото) */}
        <div
          className={`absolute inset-0 bg-gradient-to-t from-bg/90 via-bg/40 to-transparent transition-opacity duration-500 pointer-events-none ${
            hovered ? "opacity-100" : "opacity-0"
          }`}
        />

        {/* Кнопки поверх градиента — отдельный слой, не блокируют клик по фото */}
        <div
          className={`absolute bottom-0 left-0 right-0 flex flex-col items-center pb-4 gap-2 transition-all duration-500 ${
        ...
        >
            ...
            <div className="flex gap-1.5 flex-wrap justify-center px-2">
              {product.sizes.map((size) => (
                <button
                  ...
                  className={`min-w-[32px] h-7 text-[9px] font-bold tracking-wider border transition-all duration-200 ${
                    selectedSize === size
          ...
          {!isSoldOut ? (
            <button
              ...
              className={`px-8 py-2 text-[10px] font-black tracking-[0.15em] uppercase transition-all duration-300 ${
                added

...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const saved = await createPendingOrder({
        userId,
        items: serverItems,
        totalKopecks,
    ...
    try {
      const { notifyPaymentAdmin } = await import("@/lib/notify");
      const tgId = payload?.telegramId ? Number(payload.telegramId) : undefined;
      const uname = (contact as any)?.name || (contact as any)?.phone || "—";
      await notifyPaymentAdmin(savedOrderId || yooData.id, totalKopecks, uname, tgId);
    } catch {}

...
async def open_admin_panel_support(message: Message):
    ...
    await message.answer("🖥️ <b>SOULDAWN SUPPORT — Панель оператора:</b>", parse_mode="HTML", reply_markup=kb)
...
async def handle_operator_reply_click(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("✍️ <b>Введите текст ответа:</b>", parse_mode="HTML")
...
async def process_operator_reply_text(message: Message, state: FSMContext):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        await session.post("https://railway.app" + str(ticket_id) + "/reply", json={"reply": text})
    await message.answer("✅ <b>Ответ отправлен и заархивирован!</b>", parse_mode="HTML")
...
async def call_debug_menu_click(event: Message | CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔄 Проверить статус БД", callback_data="debug:test_db")]
    ])
